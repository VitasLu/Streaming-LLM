...
def get_audio_encoder(name, finetune_encoder):
    if name == "facebook/hubert-xlarge-ll60k":
        ...
        return ConvBrabchformerAudioEnoder(embed_dim=gop_dim, num_heads=audio_head, depth=audio_depth)
    elif name == "whisper_large":
        # audio_proc = WhisperProcessor.from_pretrained("../whisper/large-v2/")
        audio_proc = WhisperProcessor.from_pretrained("openai/whisper-large-v2")
        return audio_proc, WhisperAudioEnoder(model_name='../whisper/large-v2/', finetune=finetune_encoder)
    elif name == "whisper_medium":
        audio_proc = WhisperProcessor.from_pretrained("../whisper/medium")
        return audio_proc, WhisperAudioEnoder(model_name='../whisper/medium/', finetune=finetune_encoder)
    elif name == "w2v_lg_xlsr53_en":
        audio_proc = Wav2Vec2Processor.from_pretrained("../w2v_lg_xlsr53_en")
        return audio_proc, TransformerAudioEnoder(model_name='../w2v_lg_xlsr53_en', finetune=finetune_encoder)
    elif name == "hubert_lg":
        audio_proc = Wav2Vec2FeatureExtractor.from_pretrained("../hubert_lg")
        return audio_proc, TransformerAudioEnoder(model_name='../hubert_lg', finetune=finetune_encoder)
    else:
...
class WhisperAudioEnoder(nn.Module):
    def __init__(self, model_name='openai/whisper-large-v2', finetune=False): # model_name='../whisper/large-v2/'
        super().__init__()
