...
def get_qformer(name, audio_dim, llm_dim, hidden_dim, num_query_token, qformer_heads, qformer_depths):
    if name == "transformer":
        return Qformer(audio_dim, llm_dim, num_query_token, qformer_heads, qformer_depths)
    elif name == "transformer-2":
        return Qformer2(audio_dim, llm_dim, hidden_dim, num_query_token, qformer_heads, qformer_depths)
    else:
...
class Qformer(nn.Module):
    def __init__(self, audio_dim=1024, llm_dim=1536, num_query_token=10, num_heads=2, depth=3):
        super().__init__()
        self.query_token_embeds = nn.Parameter(
            torch.zeros(1, num_query_token, audio_dim)
        )
        self.qformer = Decoder(
                odim=audio_dim,
        ...
        self.connector = Mlp(in_features=audio_dim, hidden_features=audio_dim,
         out_features=llm_dim, act_layer=nn.GELU, drop=0.1)

    def forward(self, x_audio):
        x, aw = self.qformer(self.query_token_embeds, None, x_audio, None)
        x = self.connector(x)

        return x

class Qformer2(nn.Module):
    def __init__(self, audio_dim=1024, llm_dim=1536, hidden_dim=24, num_query_token=10, num_heads=2, depth=3):
        super().__init__()
        self.wav_down_mlp = Mlp(in_features=audio_dim, hidden_features=hidden_dim,
         out_features=hidden_dim, act_layer=nn.GELU, drop=0.1)
        ...
        self.connector = Mlp(in_features=hidden_dim, hidden_features=llm_dim,
         out_features=llm_dim, act_layer=nn.GELU, drop=0.1)

    def forward(self, x_audio):
        x_audio = self.wav_down_mlp(x_audio)
        x, aw = self.qformer(self.query_token_embeds, None, x_audio, None)
        x = self.connector(x)
