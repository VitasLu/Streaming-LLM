...
class MyCollator_biasingLLM:
    ...
    def __call__(self, batch):
        ...
        #uttname, audio_feat, bwords, pre_speech_prompt, post_speech_prompt, output_prompt, complete_prompt
        uttnames = [ b[0] for b in batch]
        #mel_feats = torch.stack([ b[1] for b in batch])
        audio_feat = torch.stack([ b[1] for b in batch])
        ...
        return uttnames, audio_feat, blist_tokenized, pre_tokenized_ids, post_tokenized_ids, output_tokenized_ids
...
class BiasingASRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        feat_path = data_row['feat']
        if pd.isna(audio_path):
            waveform = None
        else:
            waveform, sample_rate = torchaudio.load(audio_path)
            waveform = waveform.squeeze()
            audio_feat = torch.from_numpy(torch.load(feat_path, weights_only=False)).float().squeeze().cuda()

            '''
            data = torch.load(feat_path, weights_only=False)
            if isinstance(data, np.ndarray):
                # 如果載入的是 numpy array
                audio_feat = torch.from_numpy(data).float().squeeze().cuda()
            else:
                # 如果載入的是 torch tensor
                audio_feat = data.float().squeeze().cuda()
            '''
        
            # audio_feat = torch.from_numpy(torch.load(feat_path, weights_only=False)).float().squeeze()
        ...
        return uttname, audio_feat, waveform, target, bwords
...
class InstructionalBiasingASRDataset(BiasingASRDataset):
    ...
    def __getitem__(self, idx):
        uttname, audio_feat, waveform, target, bwords = super().__getitem__(idx)
        ...
        return uttname, audio_feat, bwords, pre_speech_prompt, post_speech_prompt, output_prompt, complete_prompt
...
if __name__ == "__main__":
    ...
    dataset = InstructionalBiasingASRDataset(json_file='./data/LibriSpeech/dev_clean_full_test.json', mode='test_utt_rand3')
    
    my_collator = MyCollator_biasingLLM(llm_tokenizer)
    
    train_loader = data_utils.DataLoader(dataset, batch_size=1, shuffle=True, collate_fn=my_collator, num_workers=1)
    uttnames, audio_feat, blist_tokenized, pre_tokenized_ids, post_tokenized_ids, output_tokenized_ids = next(iter(train_loader))
